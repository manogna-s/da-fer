...
def Train(args, model, train_dataloader, optimizer, epoch, writer):
    ...
    for step, (input, landmark, label) in enumerate(train_dataloader):
        ...
        optimizer, lr = lr_scheduler_withoutDecay(optimizer, lr=args.lr,
                                                  weight_decay=args.weight_decay)  # optimizer = lr_scheduler(optimizer, num_iter*(epoch-1)+step, 0.001, 0.75, lr=args.lr, weight_decay=args.weight_decay)
...
# def Test(args, model, source_dataloader, target_dataloader, Best_Recall, split='test'):
#     """Test."""
#
#     model.eval()
#     torch.autograd.set_detect_anomaly(True)
#
#     iter_source_dataloader = iter(source_dataloader)
...
#     LoggerInfo = '''
#     {0} Source:
#     Learning Rate {1}\n'''.format(split, args.lr)
#
#     LoggerInfo += AccuracyInfo
#
#     LoggerInfo += '''Acc_avg {0:.4f} Prec_avg {1:.4f} Recall_avg {2:.4f} F1_avg {3:.4f}
#     Total Loss {loss:.4f} Global Cls Loss {global_cls_loss:.4f} Local Cls Loss {local_cls_loss:.4f} AFN Loss {afn_loss:.4f}'''.format(
#         acc_avg, prec_avg, recall_avg, f1_avg, loss=loss.avg, global_cls_loss=global_cls_loss.avg,
#         local_cls_loss=local_cls_loss.avg, afn_loss=afn_loss.avg)
#     print(LoggerInfo)
#
...
def main():
    ...
    print('log : %s' % args.Log_Name, 'out-pth %s' % args.OutputPath, 'net: %s' % args.net,
          'pretrained %s' % args.pretrained, 'dev %s' % args.GPU_ID)

    print('Use {} * {} Image'.format(args.face_scale, args.face_scale), 'source %s' % args.source,
          'target %s' % args.target, 'train bs %d' % args.train_batch, 'test bs %d' % args.test_batch)
    ...
    if args.local_feat and args.inter_gcn and args.intra_gcn and not args.isTest:
        if args.use_cov:
            ...
            if args.use_cluster:
                ...
                Initialize_Mean_Cluster(args, train_source_dataloader, train_target_dataloader, model, True)
            else:
                print('Init Mean...')
    ...
    for epoch in range(1, args.epochs + 1):
        if args.showFeature and epoch % 5 == 1:
            Visualization('{}_Source.pdf'.format(epoch), model, train_source_dataloader, useClassify=True,
                          domain='Source')
            Visualization('{}_Target.pdf'.format(epoch), model, train_target_dataloader, useClassify=True,
                          domain='Target')
            VisualizationForTwoDomain('{}_train'.format(epoch), model, train_source_dataloader, train_target_dataloader,
                                      useClassify=True, showClusterCenter=False)
        ...
        Best_Accuracy, Best_Recall = Test(args, model, train_source_dataloader, Best_Accuracy, Best_Recall,
                                          domain='Target', split='unlabeled train')
        Test(args, model, train_target_dataloader, Best_Accuracy, Best_Recall, domain='Source', split='train')
