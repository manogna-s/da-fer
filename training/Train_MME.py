...
def Train_MME(args, model, train_source_dataloader, train_target_dataloader, optimizer, epoch, writer):
    ...
    for batch_index in range(num_iter):
        ...
        local_cls_loss_ = nn.CrossEntropyLoss()(loc_out_source.narrow(0, 0, data_source.size(0)), label_source) if args.local_feat else 0
    ...
    Total Loss {loss:.4f} Global Cls Loss {global_cls_loss:.4f} Local Cls Loss {local_cls_loss:.4f} AFN Loss {afn_loss:.4f} MME Loss {mme_loss:.4f}'''.format(acc_avg, prec_avg, recall_avg, f1_avg, loss=loss.avg, global_cls_loss=global_cls_loss.avg, local_cls_loss=local_cls_loss.avg, afn_loss=afn_loss.avg if args.use_afn else 0, mme_loss=mme_loss.avg)
