...
def Train_MCD(args, G, F_dict, train_source_dataloader, train_target_dataloader, optimizer_g, epoch,
    ...
    for batch_index in range(num_iter):
        ...
        k1, k2 = random.sample(range(0, 4), 2)
        F1_dict = F_dict[k1]
        F2_dict = F_dict[k2]
...
def Test_MCD_tsne(args, G, F_dict, dataloaders, epoch, splits=None):
    ...
    if True:
        ...
        for batch_index, (input, landmark, label) in enumerate(iter_dataloader):
            ...
            with torch.no_grad():
                ...
                combined_output = (output[0] + output[1] + output[2] + output[3])/4.0
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print('\nEvaluation ...')
        Test_MCD_tsne(args, G, F_dict, dataloaders, epoch, splits=['test_source', 'train_target', 'test_target'])

        if args.save_checkpoint and epoch%5:
            torch.save(G.state_dict(), os.path.join(args.out, f'ckpts/MCD_G_{epoch}.pkl'))
            for i in range(4):
                torch.save(F_dict[i]['cls'].state_dict(), os.path.join(args.out, f'F{i}_{epoch}.pkl'))
    writer.close()
