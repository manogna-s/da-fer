...
class Self_Attention_Classifier(nn.Module):
    def __init__(self, feature_dim = 512, num_classes=7):
        super(Self_Attention_Classifier, self).__init__()
    ...
    def forward(self, x):
        ...
        logits = attention_weights * self.cls(x)
        return logits
...
def Train_MCD_with_scn_attention(args, G, F1, F2, train_source_dataloader, train_target_dataloader, optimizer_g, optimizer_f, epoch,
              writer):
    """Train."""
    G.train()
    F1.train()
...
def main():
    ...
    F1 = Self_Attention_Classifier(feature_dim=G.output_num(), num_classes=args.class_num)
    F2 = Self_Attention_Classifier(feature_dim=G.output_num(), num_classes=args.class_num)
    F1.cuda()
    F2.cuda()
    print(F1)
    ...
    for epoch in range(1, args.epochs + 1):
        print(f'Epoch : {epoch}')
        Train_MCD_with_scn_attention(args, G, F1, F2, dataloaders['train_source'], dataloaders['train_target'], optimizer_g, optimizer_f,
                  epoch, writer)
        scheduler_g.step()
        scheduler_f.step()
        print('\nEvaluation ...')
