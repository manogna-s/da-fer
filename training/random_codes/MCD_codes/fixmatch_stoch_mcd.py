...
def Train_Stoch_Feat_MCD(args, G, F1, F2, train_source_dataloader, train_target_dataloader, optimizer_g, optimizer_f, epoch,
    ...
    for batch_index in range(num_iter):
        ...
        for i in range(num_k):
            optimizer_g.zero_grad()
            output, sigma = G(data, landmark)
            if epoch >5:
                mvn = MultivariateNormal(output, scale_tril=torch.diag_embed(sigma))
                output1 = F1(mvn.rsample())
                output2 = F2(mvn.rsample())
            else:
                output1 = F1(output)
                output2 = F2(output)
            ...
            output_s1 = output1[:batch_size, :]
            output_s2 = output2[:batch_size, :]
            logits_t1 = output1[batch_size:, :]
            logits_t2 = output2[batch_size:, :]

            loss1 = criterion(output_s1, target1)
            loss2 = criterion(output_s2, target1)
            output_t1 = F.softmax(logits_t1)
            output_t2 = F.softmax(logits_t2)
            loss_dis = torch.mean(torch.abs(output_t1 - output_t2))
            entropy_loss = -torch.mean(torch.log(torch.mean(output_t1, 0) + 1e-6))
            entropy_loss -= torch.mean(torch.log(torch.mean(output_t2, 0) + 1e-6))

            if epoch>5:
                max_probs, targets_u1 = torch.max(output_t1, dim=-1)
                mask1 = max_probs.ge(0.95).float()

                Lu_1 = (F.cross_entropy(logits_t1, targets_u1,
                                    reduction='none') * mask1).mean()

                max_probs, targets_u2 = torch.max(output_t2, dim=-1)
                mask2 = max_probs.ge(0.95).float()

                Lu_2 = (F.cross_entropy(logits_t2, targets_u2,
                                    reduction='none') * mask2).mean()
                print('Pseudo label loss:', Lu_1.data.item(), Lu_2.data.item())
                loss_dis += Lu_1 + Lu_2
