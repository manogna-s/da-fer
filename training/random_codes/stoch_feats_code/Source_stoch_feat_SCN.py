...
def get_sample_loss(G, F1, data, target):
    sample_loss = 0
    feat, sigma = G(data, sample=True)
...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default='/home/manogna/Dataset/RAF/old-folders/basic/', help='Raf-DB dataset path.')
...
class Res18_StochasticFeature(nn.Module):
    def __init__(self, pretrained = True, num_classes = 7, drop_rate = 0):
        super(Res18_StochasticFeature, self).__init__()
        self.drop_rate = drop_rate
        resnet  = models.resnet18(pretrained)
        self.penultimate_feature = nn.Sequential(*list(resnet.children())[:-2]) # before avgpool
        self.features = nn.Sequential(*list(resnet.children())[:-1]) # after avgpool 512x1

        fc_in_dim = list(resnet.children())[-1].in_features # original fc layer's in dimention 512
   
        self.fc_sigma = nn.Conv2d(512,512,(7,7))
        self.fc_sigma.apply(init_weights)

    def forward(self, x, sample=False):
        feature = self.features(x)

        penult_feature = self.penultimate_feature(x)
        sigma = None
        if sample:
            sigma = torch.flatten(self.fc_sigma(penult_feature), 1) 
...
class Classifier(nn.Module):
    def __init__(self, feature_dim = 512, num_classes = 7):
        ...
        self.cls = nn.Linear(feature_dim, num_classes) # new fc layer 512x7
...
def run_training():
    ...
    res18 = Res18_StochasticFeature(pretrained = imagenet_pretrained, drop_rate = args.drop_rate) 
    classifier = Classifier(feature_dim=512, num_classes=7)
    ...
    params = list(res18.parameters()) + list(classifier.parameters())
    ...
    for i in range(1, args.epochs + 1):
        ...
        for batch_i, (imgs, targets, indexes) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            imgs = imgs.cuda()
            features, _ = res18(imgs)
            logits = classifier(features)
            targets = targets.cuda()
            print(i, batch_i, res18.fc_sigma.weight.data.cpu()[0][0][0])
            ce_loss = criterion(logits, targets)
            sample_loss = 0
            loss_fu = 0
            if i >5:
                sample_loss, loss_fu =get_sample_loss(res18, classifier, imgs, targets)

            loss = ce_loss + 0.1 * sample_loss + 0.001 * loss_fu
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            if acc > 0.87 :
                torch.save({'iter': i,
                            ...
                            os.path.join(args.output_dir, "epoch"+str(i)+"_acc"+str(acc)+".pth"))
                print('Model saved.')
