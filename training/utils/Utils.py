...
def Build_Backbone(args):
    if args.net == 'ResNet50':
        if args.local_feat:
            model = IR_global_local(50, args.class_num)
        else:
...
def BuildModel_GCN(args):
    """Bulid Model."""

    if args.net == 'ResNet18':
        model = IR_GCN(18, args.intra_gcn, args.inter_gcn, args.rand_mat, args.all1_mat, args.use_cov,
                           args.use_cluster, args.class_num)
    elif args.net == 'ResNet50':
        model = IR_GCN(50, args.intra_gcn, args.inter_gcn, args.rand_mat, args.all1_mat, args.use_cov,
                               args.use_cluster, args.class_num)
        ...
        model = MobileNetV2(args.intra_gcn, args.inter_gcn, args.rand_mat, args.all1_mat, args.use_cov,
                                args.use_cluster, args.class_num)
...
#     if args.local_feat:
#         if args.net == 'ResNet18':
#             if args.intra_gcn or args.inter_gcn:
#                 model = IR_GCN(18, args.intra_gcn, args.inter_gcn, args.rand_mat, args.all1_mat, args.use_cov,
#                            args.use_cluster, args.class_num)
#             else:
#                 model = IR_global_local(18, args.class_num)
#         elif args.net == 'ResNet50':
#             if args.intra_gcn or args.inter_gcn:
#                 model = IR_GCN(50, args.intra_gcn, args.inter_gcn, args.rand_mat, args.all1_mat, args.use_cov,
#                                args.use_cluster, args.class_num)
...
#     else:
#         if args.net == 'ResNet18':
#             model = IR_global(18, args.class_num)
#         elif args.net == 'ResNet50':
#             model = IR_global(50, args.class_num)
#         elif args.net == 'VGGNet':
...
def BulidDataloader(args, flag1='train', flag2='source', max_samples=-1):
    """Bulid data loader."""
    ...
    if flag1 == 'train':
        ...
        if flag2 == 'target':
            if args.target == 'AISIN':
                list_patition_label = pd.read_csv(dataPath_prefix + '/%s/lists/image_list.txt' % (args.target), header=None,
                                                  delim_whitespace=True)
                list_patition_label = np.array(list_patition_label)
                n_samples = list_patition_label.shape[0]
                if max_samples == -1:
                ...
                for index in range(n_samples):
                    if list_patition_label[index, 0][:5] == "train":
                        if not os.path.exists(
                            ...
                            continue
                        img = Image.open(
                            dataPath_prefix + '/%s/images/' % (args.target) + list_patition_label[index, 0]).convert('RGB')
            ...
            if args.target == 'JAFFE':
                ...
                for index in range(list_patition_label.shape[0]):
                    ...
                    bbox = np.loadtxt(
                        dataPath_prefix + '/JAFFE/annos/bbox/' + list_patition_label[index, 0][:-4] + 'txt').astype(np.int)
                    landmark = np.loadtxt(
                        dataPath_prefix + '/JAFFE/annos/landmark_5/' + list_patition_label[index, 0][:-4] + 'txt').astype(
                        np.int)
